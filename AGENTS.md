...
Layout:
- Spec: `dashboard/tests/aggregation.spec.js` - the Total/month rollup pins (2026-08-10 regressions): counts sum, averages weight by cohort, summed lower-is-better totals band against a scaled threshold, quarter view keeps decimals and dashes data-less months. Its feed is the real Aug-2026 shape, passed to `installMockBackend(page, FEED)` and driven with the clock pinned to 2026-08-10; note the app auto-jumps to the month of the last week in the feed, so month-view specs select their month explicitly.
...
- **Dashboard deploy (the only deploy) is push-to-deploy since 2026-07-31.** Any push to `master` touching `dashboard/**` runs `.github/workflows/deploy-dashboard.yml`: it runs the E2E gate, builds the production bundle from the `VITE_TEABLE_*` **repo secrets**, and publishes `dashboard/dist` to the `gh-pages` branch (GitHub Pages source: `gh-pages`, path `/`). `workflow_dispatch` redeploys `master` on demand. Nothing needs a local `dashboard/.env` any more, and a merged PR goes live on its own - the previous hand-run `npm run deploy` was the single reason merges sat unpublished (PR #2 was merged 2026-07-30 and was still not live on 2026-07-31). `npm run deploy` from `dashboard/` still works as a break-glass path for anyone holding `dashboard/.env`, but CI is the norm; if you use it, expect CI to overwrite it on the next push.
