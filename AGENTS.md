...
The Automation rows are **Requests, Turnaround, Incident Resolve, Error Rate** - all data-driven from Teable (dash for null). For **KW27**: `Requests = 19` and `Error Rate = 5` (deduped incidents, corrected) are populated; **Turnaround + Incident Resolve are dashed** - not pullable before the meeting (self-hosted `n8n.aditor.ai` history API unauthorized/blocked; Slack incident/bot-DM reads still pending). **Never fabricate; render whatever is in Teable.** Definitions:

- **Requests / week** (Teable `automationRequests` → `automationRequests` "Requests" - new field + row, 2026-07; neutral, no thresholds yet): total weekly automation/feature requests into the pipeline (from meetings + Slack). KW27 = 19.
